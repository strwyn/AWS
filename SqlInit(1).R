...
NotePaste<-function(..., FUN = paste, sep = '\n', note.pos = 1){
  strslist<-list(...)
  ...
  #print(strslist)
  strs1<-FUN(unlist(strslist[0:(note.pos-1)][unlist(lapply(strslist[0:(note.pos-1)], 
                                                       function(i){
                                                         return(i != "" && 
                                                                  length(unlist(i)) &&
                                                                  !is.na(i))}))]),
             sep = '',
             collaspe = '')
  strs2<-FUN(unlist(strslist[-1:(-note.pos)][unlist(lapply(strslist[-1:(-note.pos)], 
                                                       function(i){ 
                                                         return(i != "" &&
                                                                  length(unlist(i)) &&
                                                                  !is.na(i))}))]),
...
GetColAgg<-function(tname, 
                    ...
                    logic = "and",
                    sep = ',', 
                    check.rows = (cond == "" || method == "" || cond.value == ""),
                    max.row = 100){
# could be really slow if too many rows in tname
  if (check.rows){
    if (CountTable(tname) > max.row){
